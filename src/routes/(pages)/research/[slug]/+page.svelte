...
<div class="flex justify-start flex-col sm:flex-row flex-wrap items-start sm:items-center gap-6 my-4">
    ...
    <img src={project.screenshots[0].url} alt={project.screenshots[0].alt} class="object-cover h-max w-full rounded-3xl overflow-hidden border border-gray-800"/>
</div>
...
<a href="{project.paperLink}" class="group">
    <div class="flex flex-row flex-wrap justify-items-start gap-2 w-full pl-4 text-xl group-hover:underline">
        {project.paperTitle}
    </div>
    <div class="flex flex-row flex-wrap justify-items-start gap-2 w-full pl-4">
...
<div class="flex flex flex-col space-y-8 items-center w-full p-4 h-full">
    {#each project.screenshots as pic, i (i)}
        {#if i > 0}
            <img src={pic.url} alt={pic.alt} style="object-fit: fit" class="h-1/3 w-full rounded-3xl overflow-hidden border border-gray-800"/>
        {/if}
    {/each}
</div>
